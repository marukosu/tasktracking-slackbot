...
class Controller:
    def __init__(self, test, parser):
        self.aware = datetime.now(timezone.utc)
        self.local_datetime = self.aware.astimezone();
    ...
    def get_start_time(self, every, at):
        ...
        return CronTab(cron_format).next(self.local_datetime.now(), default_utc=True)
    ...
    def get_next_update_time(self):
        return CronTab("0 0 * * *").next(self.local_datetime.now(), default_utc=True)
